...
const ICON_MAP = {
  Wallet, HandCoins, GraduationCap,
  Wifi, Shield, Utensils, Coffee,
  Gamepad2, Cookie, ShoppingBag, Shirt,
  Zap, CupSoda, Droplets, Home, Bus,
...
const Dashboard = ({ user }) => {
    ...
    const fetchAndSeed = async () => {
        ...
        if (snapshot.empty) {
            ...
            [...DEFAULT_INCOME].forEach(c => {
                ...
                batch.set(ref, { ...c, type: 'income', uid: user.uid, createdAt: serverTimestamp() });
            });
            [...DEFAULT_EXPENSE].forEach(c => {
                const ref = doc(collection(db, 'categories'));
                batch.set(ref, { ...c, type: 'expense', uid: user.uid, createdAt: serverTimestamp() });
            });
            await batch.commit();
        }
        
        // Listen
        const unsubscribe = onSnapshot(query(collection(db, 'categories'), where('uid', '==', user.uid), orderBy('createdAt', 'asc')), (snap) => {
  ...
  useEffect(() => {
    ...
    const fetchAccounts = async () => {
        const q = query(collection(db, 'accounts'), where('uid', '==', user.uid), orderBy('createdAt', 'asc'));
        const snapshot = await getDocs(q);
        ...
        if (snapshot.empty) {
            ...
            DEFAULT_ACCOUNTS.forEach(acc => {
                ...
                batch.set(ref, { ...acc, uid: user.uid, createdAt: serverTimestamp() });
            });
            await batch.commit();
        }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const inc = data.filter(t => t.type === 'income').reduce((acc, curr) => acc + Number(curr.amount), 0);
      const exp = data.filter(t => t.type === 'expense').reduce((acc, curr) => acc + Number(curr.amount), 0);
      setIncome(inc);
      setExpense(exp);
      setLoading(false);
    }, (error) => {
  ...
  const handleDeleteCategory = async (catId) => {
    if (!window.confirm('確定要刪除此分類嗎？相關的記帳紀錄將會變為「未分類」。')) return;
    
    try {
        const batch = writeBatch(db);
        
        // 1. Find all transactions with this category
        const q = query(collection(db, 'expenses'), where('uid', '==', user.uid), where('category', '==', catId));
        const snapshot = await getDocs(q);
  ...
  const handleDeleteAccount = async (id) => {
      if (!window.confirm('確定刪除此帳戶？相關記帳將保留但歸為「未指定帳戶」。')) return;
      ...
      <nav className="navbar">
        <div className="nav-content">
          <div className="user-info">
            ...
            <div>
                <h2 className="font-bold text-lg">早安，{user.displayName || '使用者'}</h2>
      ...
      <div className="container animate-enter">
        ...
        <div className="stats-grid">
          <div className="glass-panel">
            ...
            <h3 className="text-2xl font-bold">NT$ {(income - expense).toLocaleString()}</h3>
          </div>
          <div className="glass-panel">
             <p className="text-success text-sm mb-1 flex items-center gap-sm">
                <TrendingUp size={14} /> 本月收入
            </p>
            <h3 className="text-2xl font-bold text-success">+{income.toLocaleString()}</h3>
          </div>
          <div className="glass-panel">
             <p className="text-danger text-sm mb-1 flex items-center gap-sm">
                <TrendingDown size={14} /> 本月支出
            </p>
            <h3 className="text-2xl font-bold text-danger">-{expense.toLocaleString()}</h3>
          </div>
        </div>
                 ...
                 <div className="glass-panel p-4 mb-4 bg-black/20">
                     <div className="flex gap-2 mb-4 items-end">
                         <div className="flex-1">
                            <label className="text-xs text-muted mb-1 block">帳戶名稱</label>
                            <input 
                                value={newAccName}
                         ...
                         <div className="w-24">
                            <label className="text-xs text-muted mb-1 block">初始金額</label>
                            <input 
                                type="number"
                                value={newAccBalance}
                         ...
                         <button onClick={handleAddAccount} className="btn btn-primary py-2 px-4 text-sm h-[38px]">新增</button>
                     </div>
                 </div>
             )}
                ...
                {accounts.map(acc => (
                    <div key={acc.id} className="glass-panel p-4 relative group">
                        <div className="flex justify-between items-start mb-2">
                             ...
                             {isManagingAccs && (
                                 <button onClick={() => handleDeleteAccount(acc.id)} className="text-red-400 hover:text-red-300">
                                    <X size={14} />
                                 </button>
                             )}
                        </div>
                        <p className="text-xs text-muted mb-1">{acc.name}</p>
                        <p className="font-bold text-lg">NT$ {getAccountBalance(acc).toLocaleString()}</p>
                    </div>
                ))}
             </div>
        </div>
        ...
        {/* Add Form */}
        {isAdding && (
          <form onSubmit={handleAddTransaction} className="glass-panel mb-lg animate-enter">
             <div className="flex flex-col gap-md mb-md">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    <div>
                        <label className="text-sm text-muted mb-sm block">類型</label>
                        <div className="flex gap-sm bg-black/20 p-1 rounded-lg" style={{ background: 'rgba(0,0,0,0.2)', padding: '4px', borderRadius: '8px' }}>
                            <button
                                type="button"
                                onClick={() => setType('expense')}
                                className={`w-full py-2 rounded-md text-sm transition-all ${type === 'expense' ? 'bg-danger/20 text-danger' : 'text-muted'}`}
                                style={type === 'expense' ? { background: 'rgba(248,113,113,0.2)', color: '#f87171' } : {}}
                            >
                                支出
                            </button>
                            <button
                                ...
                                className={`w-full py-2 rounded-md text-sm transition-all ${type === 'income' ? 'bg-success/20 text-success' : 'text-muted'}`}
                                style={type === 'income' ? { background: 'rgba(52,211,153,0.2)', color: '#34d399' } : {}}
                            >
                                收入
                            </button>
                        </div>
             ...
             <div className="mb-md">
                <div className="flex justify-between items-center mb-sm">
                    ...
                    <button 
                        ...
                        className="text-xs text-primary flex items-center gap-1 hover:underline"
                    >
                        <Settings size={12} /> {isManagingCats ? '完成編輯' : '編輯分類'}
                    </button>
                </div>
                    ...
                    <div className="glass-panel border-white/10 p-4 mb-2 bg-black/20">
                        <div className="flex gap-2 mb-4">
                            <input 
                                ...
                                className="input-field text-sm py-2"
                            />
                            <button type="button" onClick={handleAddCustomCategory} className="btn btn-primary py-2 px-4 text-sm whitespace-nowrap">新增</button>
                        </div>
                        <div className="categories-scroll">
                             {categories.filter(c => c.type === type).map(cat => (
                                <div key={cat.id} className="category-pill relative pr-8 justify-between">
        ...
        <div className="flex flex-col gap-sm">
          {loading ? (
            ...
            transactions.map(t => {
                ...
                // Account Name Lookup
                const accountName = accounts.find(a => a.id === t.accountId)?.name || '現金';
                ...
                return (
                    <div key={t.id} className="transaction-item group">
                        <div className="flex items-center gap-md">
                            <div 
                                ...
                                style={{ 
                                    ...
                                    background: isExpense ? 'rgba(248,113,113,0.1)' : 'rgba(52,211,153,0.1)',
                                    color: isExpense ? '#f87171' : '#34d399'
                                }}
                            >
                                {isExpense ? <CatIcon size={20} /> : <DollarSign size={20} />}
                            </div>
                            <div>
                        ...
                        <div className="flex items-center gap-md">
                            <span className={`font-bold ${isExpense ? 'text-danger' : 'text-success'}`}>
